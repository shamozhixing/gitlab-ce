...
describe 'Edit Project Settings', feature: true do
  ...
  describe 'project tools visibility selectors', js: true do
    before do
      project.team << [member, :master]
      login_as(member)
      @tools = { builds: "pipelines", repository: "tree", issues: "issues", wiki: "wiki", snippets: "snippets", merge_requests: "merge_requests" }
    end

    it 'toggles tools visibility' do
  ...
  describe 'project tools visibility pages' do
    before do
      @tools = { builds: namespace_project_pipelines_path(project.namespace, project),
                 repository: namespace_project_tree_path(project.namespace, project, project.repository.root_ref),
    ...
    it 'renders 404 if tool is disabled' do
      @tools.each do |method_name, url|
    ...
    it 'renders 404 if tools is enabled only for team members' do
