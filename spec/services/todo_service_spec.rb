...
describe TodoService, services: true do
  ...
  describe '#mark_todos_as_done' do
    ...
    context 'when some of the todos are done already' do
      before(:each) do
        @todos = []
        @todos << create(:todo, :mentioned, user: john_doe, target: issue, project: project)
        @todos << create(:todo, :mentioned, user: john_doe, target: another_issue, project: project)
      end

      it 'returns the number of those still pending' do
        TodoService.new.mark_pending_todos_as_done(issue, john_doe)

        expect(TodoService.new.mark_todos_as_done(@todos, john_doe)).to eq(1)
      end
      ...
      it 'returns 0 if all are done' do
        ...
        expect(TodoService.new.mark_todos_as_done(@todos, john_doe)).to eq(0)
      end
    end
