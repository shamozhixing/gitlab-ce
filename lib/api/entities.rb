module API
  module Entities
    ...
    class Project < Grape::Entity
      ...
      expose(:issues_enabled) { |project, options| project.feature_enabled?(:issues, options[:user]) }
      expose(:merge_requests_enabled) { |project, options| project.feature_enabled?(:merge_requests, options[:user]) }
      expose(:wiki_enabled) { |project, options| project.feature_enabled?(:wiki, options[:user]) }
      expose(:builds_enabled) { |project, options| project.feature_enabled?(:builds, options[:user]) }
      expose(:snippets_enabled) { |project, options| project.feature_enabled?(:snippets, options[:user]) }
      ...
      expose :open_issues_count, if: lambda { |project, options| project.feature_enabled?(:issues, options[:user]) && project.default_issues_tracker? }
